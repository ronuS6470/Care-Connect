...
-- OPTION (RECOMPILE): one statement serves all three roles plus several optional filters, so a
-- cached plan compiled for one combination would be reused by the others; recompiling lets each
-- call prune the branches it can't hit.
