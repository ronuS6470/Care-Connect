-- TOP (N) + ORDER BY rather than a window function — simpler for a plain "top N" cut, with no need
-- for RANK()/ROW_NUMBER() tie semantics. The handler assigns Rank from row position.
