-- Duration uses actual check-in/check-out, in decimal: DATEDIFF(SECOND, ...) is cast to DECIMAL
...
SELECT cg.Id AS CaregiverId, cgu.FirstName + ' ' + cgu.LastName AS CaregiverFullName,
       ...
       AVG(CAST(DATEDIFF(SECOND, v.ActualStartUtc, v.ActualEndUtc) AS DECIMAL(18,4)) / 3600.0) AS AverageVisitDurationHours
FROM Visits v
